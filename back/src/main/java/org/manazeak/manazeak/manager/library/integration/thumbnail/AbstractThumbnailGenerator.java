...
public abstract class AbstractThumbnailGenerator {
    ...
    public void generateThumbnails() {
        ...
        while (true) {
            ...
            List<NameIdentifierProjection> elements = getElementPacket(lastElementId, pageable);
            ...
            lastElementId = elements.get(elements.size() - 1).getIdentifier();
        }
     ...
     */
    protected abstract List<NameIdentifierProjection> getElementPacket(Long lastId, Pageable pageable);
     ...
     */
    private Runnable processThumbsPacket(List<NameIdentifierProjection> thumbProjections) {
        return () -> {
            try {
                ...
                for (NameIdentifierProjection thumb : thumbProjections) {
     ...
     * @return The pair of: elements id and thumbnail names.
     */
    private Pair<Long, String> generateThumb(NameIdentifierProjection thumbProjection, List<ThumbnailErrorDto> thumbnailErrors) {
        try {
            ...
            if (thumbName != null) {
                return Pair.of(thumbProjection.getIdentifier(), thumbName);
            }
        } catch (Exception e) {
            log.error("Error generating the thumbnail for the element : '" + thumbProjection.getName()
                    + "' for the type : " + getThumbType());
            thumbnailErrors.add(new ThumbnailErrorDto(getThumbType(), e.getMessage(), thumbProjection.getIdentifier(),
        ...
        thumbnailErrors.add(new ThumbnailErrorDto(getThumbType(), "["+ getThumbType() +"] The thumbnail wasn't found on the FS for the file : "
                + thumbProjection.getName(), thumbProjection.getIdentifier(), ThumbnailErrorTypeEnum.FILE_NOT_FOUND));
        return null;
    }
}
