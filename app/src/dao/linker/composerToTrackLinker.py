...
class ComposerToTrackLinker(AbstractDao):

    ## Create the links between tracks and composers.
    def linkArtistToTracks(self, tracksLinkedToComposers):
        loggerScan.info(str(len(tracksLinkedToComposers)) + ' composer to link.')
    ...
    def _generateRequest(self, links):
        return 'INSERT INTO app_track_composers (track_id, artist_id) VALUES {} '\
            .format(', '.join(['(%s, %s)'] * len(links)))

    ## Inserting the object into the database.
    #   @param links the links between the tracks and the composers.
