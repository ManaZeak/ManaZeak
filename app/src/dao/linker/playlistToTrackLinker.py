...
class PlaylistToTrackLinker(AbstractDao):
    ...
    def _generateRequest(self, links):
        return 'INSERT INTO app_playlist_tracks (playlist_id, track_id) VALUES {} '\
            .format(', '.join(['(%s, %s)'] * len(links)))

    ## Inserting the object into the database.
    #   @param links the links between the playlist and the tracks.
