...
class MainPageService(object):
    ...
    def getLibrariesAndPlaylistForUser(request):
        ...
        try:
            ...
            libs = []
            PlayListHelper.getPlaylistsInformation(libraries, libs)
            # Get the user playlist.
            playlists = Playlist.objects.filter(isLibrary=False)
        except UserException as e:
            return JsonResponse(
                ErrorHandler.createStandardStateMessage(
                    False, e.errorType, MainPageService.getLibrariesAndPlaylistForUser, user)
            )
