class Layout {
    ...
    _organizeLevels(levels) {
        let keys         = Object.keys(levels), // Get key number in levels
            reservedCols = [];                  // Already used cols - NB: Carefull, have to substract -1 when comparing

        for (let i = 0; i < keys.length; ++i) { // Parse levels
            let nodes          = levels[keys[i]], // Get all nodes in level
                positionCursor = 0;               // Cursor that moves from the left to the right of the level
    ...
    _getHeight(node) {
        let count      = 1,                // Init node height
            parentNode = node.getParent(); // Store parent
