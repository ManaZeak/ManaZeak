class Node {
    ...
    draw(ctx, origin, text) {
        ...
        let x      = origin.x,                       // Path starting X coordinate
            y      = origin.y,                       // Path starting Y coordinate
            height = this.style.node.height, // Node height
            width  = this.style.node.width,  // Node width
        ...
        if (this.isCandidate) {
            let coord = { x: 0, y: 0 }; // End connector coordinates
    ...
    getConnectorCoordinates() {
        let coord = { // Connectors coordinates
    ...
    isInNode(position) {
        let inside = false, // In node indicator boolean
            x = position.x, // X target
            y = position.y; // Y target
    ...
    fitString(ctx, str, maxWidth) {
      let ellipsis      = '…',                             // Ellipsis text format
          width         = ctx.measureText(str).width,      // Get str width
          ellipsisWidth = ctx.measureText(ellipsis).width; // Get ellipsis width
    ...
    getParent()   { return this.parent ? this.parent : null; }


    /*  ----  SETTERS  ----  */
    setLabel(label)           { this.label       = label;     }
    setOrigin(origin)         { this.origin      = origin;    }
    setIsCandidate(candidate) { this.isCandidate = candidate; }
}
