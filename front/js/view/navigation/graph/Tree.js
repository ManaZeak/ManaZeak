...
//        this.callback(this.layoutWidth());    
    }
    ...
    _addNode(toAdd, parent) {
        let node = new Node(toAdd, parent, this.style); // Create node
     ...
     * @description Delete a condition node in the tree
     * @param {Number} id       : Condition ID to delete
     **/
    deleteNode(id) {
        for (let i = (this.nodes.length - 1); i >= 0 ; --i) {
            if (this.nodes[i].id === id) {
                for (let j = 0; j < this.nodes[i].parent.children.length; ++j) {
                    if (this.nodes[i].parent.children[j].id === this.nodes[i].id) {
                      this.nodes[i].parent.children.splice(j, 1);
                    }
                }
                let a = this.nodes[i];
